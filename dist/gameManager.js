...
function toPublicPlayer(player, hostId) {
    return {
        id: player.socketId,
...
function reattachPlayer(room, existing, oldSocketId, newSocketId) {
...
function getSnapshot(room, voteCounts) {
    const players = Array.from(room.players.values()).map((p) => toPublicPlayer(p, room.hostId));
    return {
        code: room.code,
        hostId: room.hostId,
        phase: room.phase,
...
export function createRoom(hostSocketId, nickname) {
...
export function joinRoom(code, socketId, nickname, sessionToken) {
...
export function markDisconnected(code, socketId) {
    ...
    if (room.hostId === socketId && room.phase === "lobby") {
        const nextHost = Array.from(room.players.values()).find((p) => p.connected && p.socketId !== socketId);
        if (nextHost)
...
export function getRoomSnapshot(room, voteCounts) {
...
export function getPlayerRole(room, socketId) {
...
export function processKill(room, killerSocketId, targetSocketId) {
    if (room.phase !== "night")
        return { error: "Not night phase" };
    const killer = room.players.get(killerSocketId);
    if (!killer || !killer.alive)
        return { error: "You cannot act" };
    if (killer.role !== "traitor")
        return { error: "Only the traitor can kill" };
    if (room.nightKillTarget)
        return { error: "Target already chosen" };
    const target = room.players.get(targetSocketId);
    if (!target || !target.alive)
        return { error: "Invalid target" };
    if (targetSocketId === killerSocketId)
        return { error: "Cannot kill yourself" };
    room.nightKillTarget = targetSocketId;
    return { success: true };
}
export function startMorning(room) {
    room.phase = "morning";
...
export function castVote(room, voterSocketId, targetSocketId) {
    if (room.phase !== "voting")
        return { error: "Not voting phase" };
