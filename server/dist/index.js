...
import { assignRoles, canStartGame, castVote, checkWinCondition, createRoom, getPlayerRole, getRoom, getRoomSnapshot, joinRoom, leaveRoom, markDisconnected, processKill, resetRoomForReplay, setPhase, startMorning, startNight, startVoting, tallyVotes, ROLE_REVEAL_SECONDS, VOTE_RESULTS_SECONDS, } from "./gameManager.js";
...
function emitRoomUpdate(room, voteCounts) {
    io.to(room.code).emit("room-update", getRoomSnapshot(room, voteCounts));
}
function emitYourRole(socket, role) {
...
function attachSocketToRoom(socket, room, sessionToken) {
...
io.on("connection", (socket) => {
    ...
    socket.on("join-room", ({ code, nickname, sessionToken, }) => {
    ...
    socket.on("rejoin-request", ({ code, sessionToken, nickname }) => {
        const result = joinRoom(code, socket.id, nickname, sessionToken);
        if ("error" in result) {
            socket.emit("error", { message: result.error });
            return;
...
function beginNightPhase(room) {
    ...
    emitRoomUpdate(room);
    const traitor = Array.from(room.players.values()).find((p) => p.role === "traitor" && p.alive);
    if (!traitor) {
...
function transitionToMorning(room) {
